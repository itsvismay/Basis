...
def new_display_mesh(mesh, Ek=None):
    viewer = igl.viewer.Viewer()
    time = 0
...
def set_up_solver():
    ...
    def test():
        ...
        # print(M_f)
        # print(M_c)
        print("f", np.dot(arb_v, M_f.dot(arb_v)))
        print("c", np.dot(arb_v, M_c.dot(arb_v)))

        print("Forces")
        f_c = np.dot(mesh_H.Nc.T.dot(mesh_L.f), arb_v)
        print(f_c)
        print(np.dot(mesh_H.f, arb_v))
    test()
