...
def compute_stiffness_matrix_simple(K, B, hMesh):
    ...
    def basis_supports_cell(b, cell):
        n1_under_b = b.support_points[cell.n1.points[0]][cell.n1.points[1]]
        n2_under_b = b.support_points[cell.n2.points[0]][cell.n2.points[1]]
        n3_under_b = b.support_points[cell.n3.points[0]][cell.n3.points[1]]
        if(n1_under_b == 1 and n2_under_b == 1 and n3_under_b == 1):
            return True
        else
            return False

    def slope_over_cell(b, cell):
        if not basis_supports_cell(b1, e):
            return 0, 0

        #from here http://www.math.lsa.umich.edu/~glarose/classes/calcIII/web/13_5/planeeqn.html
        n1 = np.array([e.n1.points[0], e.n1.points[1], b.basis[e.n1.points[0], e.n1.points[1]]])
        n2 = np.array([e.n2.points[0], e.n2.points[1], b.basis[e.n2.points[0], e.n2.points[1]]])
        n3 = np.array([e.n3.points[0], e.n3.points[1], b.basis[e.n3.points[0], e.n3.points[1]]])
